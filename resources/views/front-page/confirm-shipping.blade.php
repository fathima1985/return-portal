							...
							<div class="shipping-field row pb-3">
								...
								@foreach($formFields as $key => $_value)
									<div class="form-group col-md-4 mb-4 col-sm-6 col-xs-12">								
										...
										@if($_value['type'] == 'date')
										<input type="text" name="{{$key}}" class="datepicker form-control field_{{$key}} @if($_value['required'] == 1) required @endif"  value="{{$value}}"  onfocus="focused(this)" onfocusout="defocused(this)" @if(isset($_value['readonly'])) readonly @endif @if(isset($_value['readonly'])) readonly @endif @if(isset($_value['maxLength']))maxlength="{{$_value['maxLength']}}" @endif/>
										
										@else
											<input type="{{$_value['type']}}" name="{{$key}}" class="form-control field_{{$key}} @if($_value['required'] == 1) required @endif" value="{{$value}}"  @if(isset($_value['maxLength']))maxlength="{{$_value['maxLength']}}" @endif/>
										@endif
									</div>
								@endforeach
								</div>
