...
<div class="row section-wrapper">
		...
		<form class="confirm-payment" action="post-summary" method="post">
			...
			<input type="hidden" name="payment_id" value="{{$shipmentinfo->payment_id}}" />
			<div class="card ">
									...
									@foreach($orders['items'] as $_key => $order)
										@for($i = 1;$i<=$order['quantity'];$i++)
											@php
												$key = $_key.'_'.$i
											@endphp
									<div class="product-item col-md-12 d-flex mb-4 flex-wrap @if(!isset($product[$key])) d-none @endif"  id="item-{{$key}}">
										<div class="product-thumb col-md-2 col-sm-3 col-xs-12 text-center">
											<img src="{{$order['product_thumb']}}" alt="{{$order['product_name']}}" />
										</div>
										<div class="product-information col-md-8 col-sm-8 col-xs-12" data-key={{$key}}>
											...
											@if(isset($product[$key]))
												...
												@foreach($return as $rkey => $fdata)
													<div class="form-group field_{{$rkey}} @if($rkey == 'package_opend') d-none @endif">
														...
														@php
															$selected = ''
														@endphp		
														@if(isset($product[$key][$rkey]))
															@php
																$selected = $product[$key][$rkey]
															@endphp		
														@endif
												...
												<div class="form-group">
													...
													<textarea class="form-control" name="note[{{$key}}]" rows="3">@if(isset($product[$key]['note'])) {!!$product[$key]['note']!!} @endif</textarea>
												</div>
					...
					<h2 class="h2 text-center mt-5 mb-4">{!!$lang['return_shipment_method']!!}</h2>
						<div class="card ">
							<div class="card-body px-0 pb-0">	
								...
								<div class="gls-wrapper" @if($ship_details['shiping_method'] != 'gls') style="display:none" @endif >
									...
									<div class="shipping-field row pb-3">
										...
										@foreach($formFields as $key => $_value)
											<div class="form-group col-md-4 mb-4 col-sm-6 col-xs-12">								
												...
												@if ($key == 'order_no')
													@php
														$value = $shipmentinfo['order_id']
													@endphp
												@elseif($key == 'email_address')
													@php
														$value = $shipmentinfo['order_email']
													@endphp
			...
			<div class="text-center proceed-next p-3 mt-4">
				<a href="{{url('/confirm-shipping?ret=1')}}" class="btn btn-bordered return-prev mx-3">{!!$lang['return_to_previous_step']!!}</a>
			...
			<input type="hidden" name="order_id" value="{{$shipmentinfo['order_id']}}" />
			<input type="hidden" name="order_email" value="{{$shipmentinfo['order_email']}}" />
			<textarea name="orderjson" class="d-none">{!!$orderjson!!}</textarea>
			</form>
		</div>
	</div>	
